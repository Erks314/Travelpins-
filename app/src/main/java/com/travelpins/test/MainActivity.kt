...
class MainActivity : ComponentActivity() {
        ...
        mapView?.post {
            ...
            placesToShow.forEach { place ->
                ...
                val hue = if (category != null) {
                    colorToMarkerHue(category.colorArgb)
                } else {
            ...
            map.setOnInfoWindowClickListener { marker ->
                val place = findPlaceForMarker(marker)
                if (place != null) {
            ...
            mapView?.postDelayed({
                try {
                    if (placesToShow.size == 1) {
                        ...
                        map.animateCamera(CameraUpdateFactory.newLatLngBounds(bounds, dp(55)))
                    }
                } catch (_: Exception) {
                }
            }, 150)
    ...
    private fun findPlaceForMarker(marker: Marker): Place? {
        return currentPlaces.firstOrNull { place ->
            val samePosition =
                kotlin.math.abs(place.latitude - marker.position.latitude) < 0.000001 &&
                    kotlin.math.abs(place.longitude - marker.position.longitude) < 0.000001
            samePosition && place.name == marker.title
        }
    }
    ...
    private fun refreshFilters(content: View) {
        ...
        currentCategories.forEach { category ->
            addFilterButton(container, "${category.iconKey}  ${category.name}", category.id)
        }
    }
    ...
    private fun addFilterButton(container: LinearLayout, text: String, categoryId: Long?) {
        ...
        val button = Button(this).apply {
            this.text = text
            textSize = 12f
            setTextColor(if (selected) Color.WHITE else Color.rgb(55, 55, 55))
            background = roundedBackground(
                if (selected) Color.rgb(45, 105, 225) else Color.WHITE, 14f
            )
    ...
    private fun createPlaceView(place: Place): View {
        ...
        if (category != null) {
            val categoryView = TextView(this).apply {
                text = category.name
                textSize = 12f
                setTextColor(category.colorArgb)
                setPadding(0, 0, 0, 7)
            }
            box.addView(categoryView)
        } else {
            val uncategorized = TextView(this).apply {
        ...
        val mapsButton = Button(this).apply {
            text = "MAPS"
            textSize = 11f
            setOnClickListener { openInGoogleMaps(place) }
        }

        val categoryButton = Button(this).apply {
            text = if (category == null) "CATEGORIZZA" else "CAMBIA CATEGORIA"
            textSize = 11f
            setOnClickListener { showCategoryPicker(place) }
        }

        actions.addView(
            categoryButton,
            LinearLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, 42).apply {
                rightMargin = 6
            }
        )
        ...
        actions.addView(
            mapsButton,
            LinearLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, 42)
        )
    ...
    private fun showCategoryPicker(place: Place) {
        ...
        androidx.appcompat.app.AlertDialog.Builder(this)
            ...
            .setItems(items.toTypedArray()) { _, which ->
                ...
                lifecycleScope.launch {
                    repository.assignPlaceToCategory(place.id, categoryId)
                    runOnUiThread {
                        Toast.makeText(this@MainActivity, "Categoria aggiornata", Toast.LENGTH_SHORT).show()
                    }
                }
            }
    ...
    private fun showCategoriesDialog() {
        ...
        currentCategories.forEach { category ->
            ...
            items.add("${category.iconKey}  ${category.name}  ($count)")
        }
        ...
        androidx.appcompat.app.AlertDialog.Builder(this)
            ...
            .setItems(items.toTypedArray()) { _, which ->
                if (which == currentCategories.size) {
                    ...
                    showCategoryOptions(currentCategories[which])
                }
            }
    ...
    private fun showCreateCategoryDialog() {
        ...
        var selectedColor = Color.rgb(45, 105, 225)
        ...
        androidx.appcompat.app.AlertDialog.Builder(this)
            ...
            .setPositiveButton("CREA") { _, _ ->
                ...
                lifecycleScope.launch {
                    ...
                    runOnUiThread {
                        Toast.makeText(this@MainActivity, "Categoria creata", Toast.LENGTH_SHORT).show()
                    }
                }
            }
            .setNegativeButton("ANNULLA", null)
            ...
            .setPositiveButton("ELIMINA") { _, _ ->
                lifecycleScope.launch {
                    ...
                    runOnUiThread {
                        ...
                        Toast.makeText(this@MainActivity, "Categoria eliminata", Toast.LENGTH_SHORT).show()
                    }
                }
            }
            .setNegativeButton("ANNULLA", null)
    ...
    private fun updateImportStatus(message: String) {
        runOnUiThread {
            val content = findViewById<View>(android.R.id.content)
            val status = content.findViewWithTag<TextView>("import_status")
            status?.text = message
        }
    }
    ...
    private fun createWebView() {
        ...
        val bridge = TravelPinsJsBridge(
            ...
            onImportFinished = { savedCount ->
                runOnUiThread {
                    ...
                    Toast.makeText(this, "$savedCount luoghi importati", Toast.LENGTH_SHORT).show()
                    webView.stopLoading()
                    webView.postDelayed({ showHome() }, 700)
                }
            },
            onImportError = { error ->
                runOnUiThread {
                    updateImportStatus("Si è verificato un errore.\n\n${error.message}")
        ...
        webView.webViewClient = object : WebViewClient() {
            ...
            override fun onPageFinished(view: WebView, url: String) {
                ...
                if (url.contains("consent.google.com")) {
                    if (!consentAttempted) {
                        ...
                        updateImportStatus("Autorizzazione Google in corso…")
                ...
                if (GoogleMapsScraperScript.isGoogleListUrl(url)) {
                    ...
                    if (currentListId != null && !scanStarted) {
                        scanStarted = true
                        view.postDelayed({ scanGoogleList() }, 500)
                    }
                }
            }
            ...
            override fun onRenderProcessGone(
                ...
                updateImportStatus("Google Maps ha interrotto l'importazione.")
                return true
            }
    ...
    private fun extractListId(url: String): String? {
        ...
        Regex("""/local/userlists/list/([^?/]+)""", RegexOption.IGNORE_CASE).find(url)?.let {
            return it.groupValues[1]
        }
        Regex("2s([A-Za-z0-9_-]{20,})").find(url)?.let {
            return it.groupValues[1]
        }
        return null
    }
    ...
    private fun acceptGoogleConsent() {
        ...
        updateImportStatus("Autorizzazione Google…")
    ...
    private fun scanGoogleList() {
        ...
        updateImportStatus("Lettura dei luoghi in corso…")
    ...
    private fun handleGoogleIntent(intentUrl: String) {
        try {
            ...
            if (start == -1) {
                updateImportStatus("Impossibile aprire la lista Google Maps.")
    ...
    private fun handleIntent(intent: Intent?) {
        ...
        if (text.isNullOrBlank()) {
            Toast.makeText(this, "Nessun testo ricevuto", Toast.LENGTH_SHORT).show()
            return
        }
        ...
        if (match == null) {
            Toast.makeText(this, "Nessun URL trovato", Toast.LENGTH_SHORT).show()
            return
        }
        ...
        updateImportStatus("Apertura della lista Google Maps…")
