...
object PlaceDetailsParser {
    ...
    fun parse(rawJson: String): PlaceDetails? {
        return try {
            ...
            fun walk(node: Any?) {
                ...
                if (node is JSONArray) {
                    for (i in 0 until node.length()) {
                        ...
                        // 1. Foto: URL lh3.googleusercontent.com
                        ...
                        // 2. Rating: Double tra 1.0 e 5.0 (prima occorrenza plausibile)
                        if (item is Double && item >= 1.0 && item <= 5.0 && rating == null) {
                            rating = item
                        }
                        
                        // 3. Review count: Int > 10 (prima occorrenza plausibile)
                        if (item is Int && item > 10 && reviewCount == null) {
                            reviewCount = item
                        }
                        
                        // 4. Website: stringa http/https che non è google
                        if (item is String && (item.startsWith("http://") || item.startsWith("https://")) && 
                            !item.contains("google.com") && websiteUrl == null) {
                            websiteUrl = item
                        }
                        
                        // 5. Testo lungo: descrizione o recensione
            ...
            // Deduplica e limita le foto per performance
            val uniquePhotos = photos.distinctBy { it.url }.take(10)
